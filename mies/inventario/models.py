...
class EquipoCabecera(models.Model):
    ...
    capacidadDisco = models.ForeignKey(CapacidadDisco, verbose_name="Disco Duro",on_delete=models.PROTECT, null=True,blank=True)
    capacidadMemoria = models.ForeignKey(CapacidadMemoriaRam,verbose_name="Memoria Ram", on_delete=models.PROTECT, null=True,blank=True)
    capacidadProcesador = models.ForeignKey(Procesador,verbose_name="Procesador", on_delete=models.PROTECT, null=True,blank=True)
    foto = models.ImageField('Foto', upload_to='Equipo/%Y/%m/%d/', height_field=None, width_field=None, max_length=None,null=True,blank=True)
    ...
    def toJSON(self):
        item = model_to_dict(self)
        item['det']= [i.toJSON() for i in self.equipodetalle_set.all()]
        return item
    def __str__(self):
        """Unicode representation of InvetarioDistritoCabecera."""
        return ("{}- {}- {}").format(self.descripcion,self.serie, self.codigoMies)
